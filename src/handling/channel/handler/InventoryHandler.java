...
public class InventoryHandler {
    ...
    public static final void UseItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        //89 00 [75 E4 2E 02] [02 00] [A0 25 26 00]
        public static final void UseSpecialItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || !chr.isAlive() || chr.getMapId() == 749040100 || chr.getMap() == null || chr.hasDisease(MapleDisease.POTION) || chr.hasBlockedInventory() || chr.inPVP()) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        ...
        if (!FieldLimitType.PotionUse.check(chr.getMap().getFieldLimit())) { //cwk quick hack
            ...
            case 2500003: {
                  if(chr.getLevel() < 11) {
                      c.getSession().write(CField.getGameMessage("You must be above level 10 to use this item.", true));
                      c.getSession().write(CWvsContext.enableActions());
                  }
                 final Map<Skill, SkillEntry> skills = new HashMap<>(c.getPlayer().getSkills());
                 final Map<Skill, SkillEntry> newList = new HashMap<>();
                 for (Entry<Skill, SkillEntry> session : skills.entrySet()) {
                     newList.put(session.getKey(), new SkillEntry((byte) 0, (byte) 0, -1));
                 }
                 chr.changeSkillsLevel(newList);
                 newList.clear();
                 skills.clear();
                 chr.setSP((chr.getLevel() - 10) * 3);
                 break;
            }
