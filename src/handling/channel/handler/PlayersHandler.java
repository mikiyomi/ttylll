...
public class PlayersHandler {
    ...
    public static final void updateStolenSkill(final LittleEndianAccessor slea, final MapleClient c) {
        //2D 46 0F 00 10 27 01 00 00
        SkillSwipe swipe;
        int skillid = slea.readInt();
        int victim = slea.readInt();
        byte mode = slea.readByte(); // 00
        Skill skill = SkillFactory.getSkill(skillid);
        int jobIndex;
        if (skill.isFourthJobSkill(skillid)) { //this is shit coding right here
            jobIndex = 4;
        } else if (skill.isThirdJobSkill(skillid)) {
            jobIndex = 3;
        } else if (skill.isSecondJobSkill(skillid)) {
            jobIndex = 2;
        } else {
            jobIndex = 1;
        }
        int victimskilllevel = c.getChannelServer().getPlayerStorage().getCharacterById(victim).getSkillLevel(skillid);
        //c.getSession().write(CWvsContext.stealSkill(mode, jobIndex, swipe.getSlot(), skillid, victimskilllevel));
        for (int i = 0; i < jobIndex; i++) {
       // c.getPlayer().updateSkillSwipe(i, swipe);
        }
    }
    ...
    public static final void viewSkills(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.getSession().write(CWvsContext.showTargetSkills(true, victim, jobid, c.getChannelServer().getPlayerStorage().getCharacterById(victim).getSkills(), c));
    }
