...
public class PacketHelper {
    ...
    public static final void addCharStats(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr) {
        ...
        mplew.writeShort(chr.getSubcategory()); //1 here = db, 2 = cannoner, 10 = Jett
        ...
        // from here, if got part time job, below = 0
        ...
        mplew.write(5); // 5 = new characters for non jett, 4 jett here, 12 = idk?..
        mplew.writeInt(0); // jump?, 0xA000000 here jett, 0x8000001 = no part time job

        mplew.write(0); // Part time job id, 1-5
        mplew.writeReversedLong(getTime(-2)); // I really don't know how to make this show time.
    ...
    public static final void addCharLook(final MaplePacketLittleEndianWriter mplew, final MapleCharacterLook chr, final boolean mega) {
        ...
        //03 A2 98 0F 00 04 8E BF 0F 00 05 C6 0E 10 00 07 8A 5D 10 00 08 CB 83 10 00 09 D7 D1 10 00 0B 9B 60 17 00 0C D4 F9 10 00 0D 5C FA 10 00 0F 58 FB 10 00 10 67 FA 10 00 11 DE 1E 11 00 31 52 6E 11 00 32 3D 46 11 00 33 3D 94 11 00 37 20 E2 11 00 FF FF 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 44 FE CA FF 00 00 00 00 00 01 00 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
    ...
    public static final void addCharacterInfo(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr) {
        ...
        mplew.writeShort(0); // For each one short
    ...
    public static final void addStealSkills(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr) {
        for (int i = 1; i <= 4; i++) {
            for (int z = 0; z < getSkillBook(i); z++) {
                mplew.writeInt(0); // phantom steal skills list
            }
        }

        // equipped stolen skills
        mplew.writeInt(0); // first job
        mplew.writeInt(0); // second job
        mplew.writeInt(0); // third job
        mplew.writeInt(3121002); // fourth job
    }

    public static final void addInnerStats(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr) {
        mplew.writeShort(0);
        //for (int i = 0; i < skills.size(); ++i) {
        //    mplew.write(i + 1); // key
        //    mplew.writeInt(0); // id 7000000 id ++, 71 = char cards
        //    mplew.write(0); // level
        //    mplew.write(0); // rank, C, B, A, and S
        //}        
        mplew.writeInt(0); // honor level
        mplew.writeInt(0); // honor exp
    }
    ...
    public static <E extends Buffstat> void writeSingleMask(MaplePacketLittleEndianWriter mplew, E statup) {
        for (int i = GameConstants.MAX_BUFFSTAT; i >= 1; i--) { // 4 LOL its 16 in normal sources4bytes? O_o yea i guess  olook what i got
