...
public class CWvsContext {
    ...
    public static class BuffPacket {
        ...
        public static byte[] giveEnergyChargeTest(int bar, int bufflength, int targets) {
            return giveEnergyChargeTest(-1, bar, bufflength, targets);
        }

        public static byte[] giveEnergyChargeTest(int cid, int bar, int bufflength, int targets) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            //00 00 00 00 00 00 00 00 00 00 00 00 AA 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 04
            if (cid == -1) {
                mplew.writeShort(SendPacketOpcode.GIVE_BUFF.getValue());
            } else {
                mplew.writeShort(SendPacketOpcode.GIVE_FOREIGN_BUFF.getValue());
        ...
        public static byte[] giveAriaBuff(int bufflevel, int buffid, int bufflength) {
            ...
            mplew.writeShort(1000); // not sure..
            mplew.write(0);
            System.out.println("ARIRA_BUFF PACKET: " + mplew.toString());
            return mplew.getPacket();
        }
//00 00 00 00 00 00 00 80 00 00 00 00 00 00 00 00 00 00 40 00 00 00 00 00 00 00 00 00 00 00 00 00 [14 00] [AC 0E 70 01]
        //[10 98 02 00] [14 00] [AC 0E 70 01] [10 98 02 00] 00 00 00 E8 03 00
        public static byte[] giveBuff(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect) {
            ...
            if (statups.containsKey(MapleBuffStat.UNKNOWN8)) { // 0x20, 7
                mplew.writeInt(0);
            }
            mplew.write(0);
            mplew.write(0);
            ...
            if (statups.containsKey(MapleBuffStat.DAMAGE_BUFF) && statups.containsKey(MapleBuffStat.MP_BUFF)
                ...
                mplew.write(0); // related to jaguar-oshi
            } else if (statups.containsKey(MapleBuffStat.DICE_ROLL)) {
                mplew.writeInt(GameConstants.getDiceStat(buffid, 3));
                mplew.writeInt(GameConstants.getDiceStat(buffid, 3));
                mplew.writeInt(GameConstants.getDiceStat(buffid, 4));
                mplew.writeZeroBytes(20); // ints
                mplew.writeInt(GameConstants.getDiceStat(buffid, 2));
                mplew.writeZeroBytes(12); // ints
                mplew.writeInt(GameConstants.getDiceStat(buffid, 5));
                mplew.writeZeroBytes(16); // ints
                mplew.writeInt(GameConstants.getDiceStat(buffid, 6));
                mplew.writeZeroBytes(16); // ints
            } else if (statups.containsKey(MapleBuffStat.DIVINE_SHIELD)) { // effect.isDivineShield()
                mplew.writeInt(0); // effect.getEnhancedWatk()
                ...
                mplew.writeInt(0);
            } else if (statups.containsKey(MapleBuffStat.UNKNOWN10)) {
                mplew.writeInt(0);
            } else if (statups.containsKey(MapleBuffStat.COMBO) || statups.containsKey(MapleBuffStat.SUMMON)) {
                mplew.write(0);
            }
            //if (effect != null && effect.getCharColor() > 0) {
            //	mplew.writeInt(effect.getCharColor());
            //} else if (effect != null && effect.isInflation()) {
            //	mplew.writeInt(effect.getInflation());
            //}
            mplew.writeShort(0); // delay?, haste/shadow stars = 1000
            mplew.write(0);
            if (isMovementAffectingStat(statups)) {
                mplew.write(4); // updatePlayerStats
            }

            return mplew.getPacket();
        }

        private static boolean isMovementAffectingStat(final Map<MapleBuffStat, Integer> statups) {
            if (statups.containsKey(MapleBuffStat.JUMP) || statups.containsKey(MapleBuffStat.MORPH)
                    || statups.containsKey(MapleBuffStat.GHOST_MORPH) || statups.containsKey(MapleBuffStat.MAPLE_WARRIOR)
                    || statups.containsKey(MapleBuffStat.MONSTER_RIDING) || statups.containsKey(MapleBuffStat.DASH_SPEED)
                    || statups.containsKey(MapleBuffStat.DASH_JUMP) || statups.containsKey(MapleBuffStat.SOARING)
                    || statups.containsKey(MapleBuffStat.YELLOW_AURA) || statups.containsKey(MapleBuffStat.SNATCH)
                    || statups.containsKey(MapleBuffStat.ANGEL_SPEED) || statups.containsKey(MapleBuffStat.ANGEL_JUMP)
                    || statups.containsKey(MapleBuffStat.ENERGY_CHARGE) || statups.containsKey(MapleBuffStat.MECH_CHANGE)) {
                return true;
            }
            return false;
            // 0x20000, 1
    ...
    public static byte[] updatePlayerStats(final Map<MapleStat, Integer> mystats, final boolean itemReaction, final MapleCharacter chr) {
        ...
        for (final Entry<MapleStat, Integer> statupdate : mystats.entrySet()) {
            switch (statupdate.getKey()) {
                ...
                case AVAILABLEAP:
                    ...
                    break;
                case AVAILABLESP:
                    if (GameConstants.isEvan(chr.getJob()) || GameConstants.isResist(chr.getJob()) || GameConstants.isMercedes(chr.getJob()) || GameConstants.isJett(chr.getJob()) || GameConstants.isPhantom(chr.getJob())) {
                        mplew.write(chr.getRemainingSpSize());
                        for (int i = 0; i < chr.getRemainingSps().length; i++) {
                            if (chr.getRemainingSp(i) > 0) { // nice its here, so.. lets test  make ur self gm ok
                                mplew.write(i + 1);
        ...
        mplew.write(0); // SetSecondaryStatChangedPoint [byte], increment after every tickcount?..
    ...
    public static byte[] stealSkill(final int mode, int jobIndex, int slot, int skillid, int level) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.UPDATE_STOLEN_SKILLS.getValue());
        mplew.write(1); // Tick
        mplew.write(mode);
        // 0 : Success and Update
        // 1 : There aren't any targets to steal from.
        // 2 : Failed to steal due to an unknown reason.
        // 3 : Delete
        // 4/5 : Silent update?
        switch (mode) {
            case 0:
                mplew.writeInt(jobIndex); // Job index
                mplew.writeInt(slot); // Slot
                mplew.writeInt(skillid); // Skill Id
                mplew.writeInt(level); // Skill Level
                mplew.writeInt(0); // Expiration? , idk, shouldn't be an int
                break;
            case 3: // Delete
                mplew.writeInt(jobIndex); // Job index
                mplew.writeInt(slot); // Slot
    ...
    public static byte[] showTargetSkills(boolean empty, int victimid, int jobid, final Map<Skill, SkillEntry> update, MapleClient c) { // Display target skill menu
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
//[01] [A3 CC 00 00] [04 00 00 00] [E8 00 00 00] [13 00 00 00] 5D 43 23 00 4D 1C 23 00 6E 6A 23 00 6C 88 1E 00 6D 88 1E 00 70 6A 23 00 69 6A 23 00 61 43 23 00 59 43 23 00 72 6A 23 00 5A 43 23 00 4A 1C 23 00 5B 43 23 00 4B 1C 23 00 6A 88 1E 00 5C 43 23 00 4C 1C 23 00 6D 6A 23 00 6B 88 1E 00
        //int i = c.getChannelServer().getPlayerStorage().getCharacterById(victimid).getSkills().size();
        ...
        mplew.writeInt(4); // Show mode
        mplew.writeInt(jobid);
        mplew.writeInt(c.getChannelServer().getPlayerStorage().getCharacterById(victimid).getSkills().size());
        for (final Entry<Skill, SkillEntry> z : update.entrySet()) {
            mplew.writeInt(z.getKey().getId());
        }
    ...
    public static byte[] updateEquippedSkill(final int skillId, final int stolenId) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.EQUIPPED_SKILL.getValue());
        mplew.write(1); // Tick
        mplew.write(stolenId > 0 ? 1 : 0);
        mplew.writeInt(skillId); // original phantom skill
        if (stolenId > 0) {
            mplew.writeInt(stolenId);
        }
