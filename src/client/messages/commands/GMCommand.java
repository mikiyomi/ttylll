...
*/
    ...
    public static class Trophy extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            final int itemId = 4000038;
            final short quantity = (short) CommandProcessorUtil.getOptionalIntArg(splitted, 2, 1);
            MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            Item item;
            item = new client.inventory.Item(itemId, (byte) 0, quantity, (byte) 0);
            item.setOwner(c.getPlayer().getName());
            item.setGMLog(c.getPlayer().getName() + " used !trophy" + quantity + " " + victim.getName());
            MapleInventoryManipulator.addbyItem(victim.getClient(), item);
            victim.dropMessage(5, "You have recieved " + quantity + " trophy. Please use @coco to exchange it for an Item Of Choice.");
            c.getPlayer().dropMessage(5, victim.getName() + " has recieved " + quantity + " trophy.");
            if (quantity > 1) {
                World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "[Event] " + victim.getName() + " has won " + quantity + " Event Trophies from " + c.getPlayer().getName() + "'s Event!"));
            } else {
                World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "[Event] " + victim.getName() + " has won an Event Trophy from " + c.getPlayer().getName() + "'s Event!"));
            }
            return 1;
        }
    }
    ...
    public static class MuteMap extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            for (MapleCharacter chr : c.getPlayer().getMap().getCharacters()) {
                ...
                chr.dropMessage(5, "This map's chatting ability has been changed, please listen as a GM gives instructions.");
            }
     ...
     public static class Mute extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
                ...
                victim.dropMessage(5, "Your chatting ability has been changed by " + c.getPlayer().getName());
            c.getPlayer().dropMessage(6, "Done");
            return 1;
        }
    }
   // public static class SeduceMap extends CommandExecute {
...
//int level = Integer.parseInt(splitted[1]);
     ...
     * MapleEvent.onStartEvent(c.getPlayer()); return 1; }
    }
     */
   ...
   //     @Override
    //    public int execute(MapleClient c, String[] splitted) {
     //       c.getPlayer().dropMessage(6, "Top Warpped Out.");
   ...
   //event shit   // //         if (person.getPosition().y <= -206 && !person.isGM()) {
  ...
  //  public static class warpoxleft extends CommandExecute {
   ...
   //     @Override
   //     public int execute(MapleClient c, String[] splitted) {
           // c.getPlayer().dropMessage(6, "Left Warpped Out.");
     //       for (MapleMapObject wrappedPerson : c.getPlayer().getMap().getCharactersAsMapObjects()) {
      //          MapleCharacter person = (MapleCharacter) wrappedPerson;
   ...
   /* public static class warpox extends CommandExecute {
    ...
    public static class warpoxright extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getPlayer().dropMessage(6, "Right Warpped Out.");
            for (MapleMapObject wrappedPerson : c.getPlayer().getMap().getCharactersAsMapObjects()) {
                MapleCharacter person = (MapleCharacter) wrappedPerson;
    ...
    public static class warpoxmiddle extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getPlayer().dropMessage(6, "Middle Warpped Out.");
            for (MapleMapObject wrappedPerson : c.getPlayer().getMap().getCharactersAsMapObjects()) {
                MapleCharacter person = (MapleCharacter) wrappedPerson;
